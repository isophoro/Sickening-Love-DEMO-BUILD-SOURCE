...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'first-sight':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/first-sight/dia'));
				...
				case 'trees':
					{
						defaultCamZoom = 0.9;
						curStage = 'trees';
						var bg:FlxSprite = new FlxSprite(-508.55, -100.3).loadGraphic(Paths.image('mochibg/back'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);

						var stageFront:FlxSprite = new FlxSprite(2.15, -120.15).loadGraphic(Paths.image('mochibg/back trees'));
						stageFront.antialiasing = true;
						stageFront.scrollFactor.set(0.9, 0.9);
						stageFront.active = false;
						add(stageFront);

						var stageCurtains:FlxSprite = new FlxSprite(-480.7, -210.2).loadGraphic(Paths.image('mochibg/ground'));
						stageCurtains.antialiasing = true;
						stageCurtains.scrollFactor.set(0.9, 0.9);
						stageCurtains.active = false;
						add(stageCurtains);
						
						var benches:FlxSprite = new FlxSprite(17.1, 646.1).loadGraphic(Paths.image('mochibg/benches'));
						benches.antialiasing = true;
						benches.scrollFactor.set(0.9, 0.9);
						benches.active = false;
						add(benches);

						clowfoe = new FlxSprite(-8, 460);
						clowfoe.frames = Paths.getSparrowAtlas('cameos/clowfoe');
						clowfoe.animation.addByPrefix('bump', 'bench 1', 24);
						clowfoe.antialiasing = true;
						clowfoe.scrollFactor.set(0.9, 0.9);
						add(clowfoe);
						clowfoe.animation.play('bump');

						bb = new FlxSprite(1412, 404);
						bb.frames = Paths.getSparrowAtlas('cameos/bbAndVapurr');
						bb.animation.addByPrefix('bump', 'bench 2', 24);
						bb.flipX = true;
						bb.antialiasing = true;
						bb.scrollFactor.set(0.9, 0.9);
						add(bb);
						bb.animation.play('bump');
		...
		if (!PlayStateChangeables.Optimize)
		{
			...
			switch(curStage){
				case 'trees':
					var bushes:FlxSprite = new FlxSprite(-279.3, 795.8).loadGraphic(Paths.image('mochibg/bushes'));
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					if (dad.curCharacter.toLowerCase() == 'mochi')
					{
						if (daNote.isSustainNote)
						{
							delay = 4;
						...
						{
							case 2:
								dad.playAnim('singUP' + altAnim, true, false, delay);
							case 3:
								dad.playAnim('singRIGHT' + altAnim, true, false, delay);
							case 1:
								dad.playAnim('singDOWN' + altAnim, true, false, delay);
							case 0:
								dad.playAnim('singLEFT' + altAnim, true, false, delay);
						}
					}
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (curSong.toLowerCase() == "first-sight") 
					{
						FlxG.switchState(new VideoState('assets/videos/funnyVideo.webm', new StoryMenuState()));
					}else{
						FlxG.sound.playMusic(Paths.music('freakyMenu'));
					}
